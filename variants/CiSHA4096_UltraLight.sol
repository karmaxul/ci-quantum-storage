...
/**
 * @title CiSHA4096 - Ultra-Light Variant
 * @notice 32 rounds, 8-state core, minimal operations
 *         Target: ~300k - 450k gas (comparable to heavy DeFi swaps)
 */
contract CiSHA4096_UltraLight {
    uint256 constant ROUNDS = 32;
    uint256 constant CI_NUM = 85;
    uint256 constant CI_DEN = 27;

    function ciSha4096(bytes calldata data) public pure returns (bytes32[8] memory) {
        uint256[8] memory state = [
    ...
    function gasBenchmark(bytes calldata message) external view returns (uint256 gasUsed, bytes32[8] memory hash) {
